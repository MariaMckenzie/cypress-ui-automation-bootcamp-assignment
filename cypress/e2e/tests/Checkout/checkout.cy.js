...
describe("Checkout Flow", () => {
    ...
    it("should complete a checkout flow with multiple items (from product page)", () => {        
        // variables
        let itemInfo = [
            [item3.no, item3.name, item3.price],
        ...
        // go to product page (item 1)
        inventoryPage.goToProductPage(itemInfo[0][0])
        ...
        productPage.goToInventory()

        // go to product page (item 2)
        inventoryPage.goToProductPage(itemInfo[1][0])
    ...
    it("should not complete a checkout flow without completing the shipping form", () => {        
        // variables
        let itemInfo = [
            [item3.no, item3.name, item3.price],
            [item6.no, item6.name, item6.price],
        ...
        // continue to step two and attempt to checkout with all data missing
        checkoutPage.continueToStepTwo(user5.firstname, user5.lastname, user5.postal)
        // assert that the user cannot move on to step two
        cy.get(checkoutPage.errorMessage).should("contain", user5.errorMsg)


        // continue to step two and attempt to checkout with firstname missing
        checkoutPage.continueToStepTwo(user2.firstname, user2.lastname, user2.postal)
        // assert that the user cannot move on to step two
        cy.get(checkoutPage.errorMessage).should("contain", user2.errorMsg)
         // clear form
        checkoutPage.clearForm()
         
 
        // continue to step two and attempt to checkout with lastname missing
        checkoutPage.continueToStepTwo(user3.firstname, user3.lastname, user3.postal)
        // assert that the user cannot move on to step two
        cy.get(checkoutPage.errorMessage).should("contain", user3.errorMsg)
        // clear form
       checkoutPage.clearForm()

 
       // continue to step two and attempt to checkout with postalcode missing
       checkoutPage.continueToStepTwo(user4.firstname, user4.lastname, user4.postal)
       // assert that the user cannot move on to step two
       cy.get(checkoutPage.errorMessage).should("contain", user4.errorMsg)
       // clear form
      checkoutPage.clearForm()
